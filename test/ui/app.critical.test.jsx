...
describe('UI crítica (React): auth, errores y navegación', () => {
  ...
  it('Login: header y bottom nav no interceptan punteros cuando el shell es no interactivo', async () => {
    ...
    const shells = document.querySelectorAll('[data-waitme-screen-shell]')
    const headers = document.querySelectorAll('header[data-waitme-header]')
    const navs = document.querySelectorAll('nav[data-waitme-nav]')
